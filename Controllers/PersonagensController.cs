    ...
    public class PersonagensController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByPerfilAsync(int userId)
        {
            try
            {
                ...
                if (usuario!.Perfil == "Admin") lista = await _context.TB_PERSONAGENS.ToListAsync();
                else lista = await _context.TB_PERSONAGENS.Where(p => p.Usuario!.Id == userId).ToListAsync();
                return Ok(lista);
            }
            catch (System.Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                if (pRemover == null) throw new Exception("Personagem não encontrado.");
